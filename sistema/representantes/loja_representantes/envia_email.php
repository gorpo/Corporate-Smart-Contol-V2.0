...
$representante =  mb_convert_case($_SESSION['nome'],MB_CASE_LOWER,mb_detect_encoding($_SESSION['nome'])) ;
...
$pdo = Database::conectar();
$sql = "SELECT * FROM carrinho_representante_steve ORDER BY id DESC limit 1";
foreach($pdo->query($sql)as $row){
    $id = $row['id'];
    $data = $row['data'];
}
...
foreach($pdo->query($sql)as $row){
    ...
    $array_referencia = explode(',', $referencia);
    $array_cor = explode(',', $cor);
    $array_tamanho = explode(',', $tamanho);
...
//$mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
    ...
    echo "Seu pedido foi computado mas tivemos um erro ao enviar o email, informe a nossa equipe o codigo de erro: {$mail->ErrorInfo}";
}
